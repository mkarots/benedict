...
from .command_definitions import CommandDefinition, CommandType, CommandIntent, COMMAND_DEFINITIONS
...
class CommandClassifier:
        ...
        for cmd_def in self.command_definitions:
            ...
            for pattern in cmd_def.patterns:
                try:
                    ...
                    logger.warning(
                        f"Invalid regex pattern '{pattern}' for command {cmd_def.name}: {e}"
                    )
    ...
    def classify(self, text: str) -> Optional[CommandIntent]:
        ...
        if best_match and best_confidence >= 0.5:
            logger.debug(
                f"Classified '{text}' as {best_match.command_type.value} (confidence: {best_confidence:.2f})"
            )
            return best_match
    ...
    def _match_command(
        self, cmd_def: CommandDefinition, text_lower: str, text_original: str
    ...
    def _extract_parameters(
        self, cmd_def: CommandDefinition, groups: tuple, text_original: str
    ) -> Dict[str, Any]:
    ...
    def _calculate_confidence(
        self, cmd_def: CommandDefinition, text_lower: str, matched_pattern: str
    ) -> float:
