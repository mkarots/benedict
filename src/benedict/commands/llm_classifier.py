...
class LLMCommandClassifier:
    ...
    def classify(
        ...
        try:
            ...
            if tool_calls:
                logger.info(
                    f"LLM returned {len(tool_calls)} tool calls: {[tc.get('name') for tc in tool_calls]}"
                )
                return {"tool_calls": tool_calls}

            # No tool calls - treat as query if fallback enabled
            logger.info(
                f"No tool calls detected for '{text}', treating as query. Response was: {str(response)[:200]}"
            )
            if self.fallback_to_query:
                return None
    ...
    def _call_llm_with_tools(
        self, prompt: str, functions: List[Dict], user_text: str
    ) -> Dict[str, Any]:
        ...
        try:
            response = self.llm.generate(
                messages=messages, system=prompt, tools=functions, max_tokens=2000
            )
    ...
    def _parse_response(self, response: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # Handle OpenAI function_call format (legacy)
            ...
            tool_calls.append({"name": func_call.get("name"), "arguments": arguments})
